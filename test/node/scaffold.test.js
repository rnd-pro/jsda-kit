...
describe('scaffold', () => {
  ...
  it('should create .npmrc', () => {
    assert.ok(fs.existsSync(path.join(tmpDir, '.npmrc')));
  });
  ...
  it('should create dynamic index.html.js with applyData + wcSsr', () => {
    let file = path.join(tmpDir, 'src/dynamic/index.html.js');
    assert.ok(fs.existsSync(file));
    let content = fs.readFileSync(file, 'utf8');
    assert.ok(content.includes('wcSsr'), 'Should use wcSsr');
